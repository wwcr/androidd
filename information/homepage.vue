...
export default {
    ...
    methods: {
        ...
        details (val) { //单个数据详情页
            location.href = '#/informationdetails?id=' + val + '&msg='+ 2;
        },
        //   onRefresh(done) {
        //         this.getlist(2);
        ...
        //     },
             getlist:function (id) {
                ...
                this.$http.post(units.host('get_single_contentlist','api'), units.paramsno({
                    uid,
    ...
    created () {
        ...
        if (cookie.get("user_id") == uid) {
            ...
            units.title.set('我的主页');
        } else {
            units.title.set('TA的主页');
        }
